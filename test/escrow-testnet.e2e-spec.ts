...
async function waitForPaymentStatus(
  ...
  throw new Error(`Payment ${paymentId} did not reach '${expected}' within ${timeoutMs}ms`);
}
...
describe("XRPL Escrow Testnet (풀스택 통합 테스트)", () => {
  ...
  beforeAll(async () => {
    ...
    escrowPaymentsService = module.get<EscrowPaymentsService>(EscrowPaymentsService);
  ...
  it(
    "초기금 에스크로 생성 → 이벤트 2개 양방향 승인 완료 시 자동 EscrowFinish",
    async () => {
      // ── 1. 결제 내역 생성 ─────────────────────────────────────────────────
      ...
      expect(
        payment.escrows[0].approvals.every((a) => !a.buyerApproved && !a.sellerApproved),
      ).toBe(true);

      // ── 2. 결제 양측 승인 → APPROVED (XRPL 미실행) ───────────────────────
      await escrowPaymentsService.approvePayment(paymentId, "buyer");
      const afterBothApprove = await escrowPaymentsService.approvePayment(paymentId, "seller");
      ...
      const escrowItem = await waitForEscrowStatus(
        escrowPaymentsService, paymentId, escrowItemId, "ESCROWED", 60_000,
      );
      const activePayment = await waitForPaymentStatus(
        escrowPaymentsService, paymentId, "ACTIVE", 5_000,
      );
      ...
      const afterE1Buyer = await escrowPaymentsService.approveEvent(
        paymentId, escrowItemId, "SHIPMENT_CONFIRMED", "buyer",
      );
      const e1b = afterE1Buyer.escrows.find((e) => e._id.toString() === escrowItemId)!;
      const approval1b = e1b.approvals.find((a) => a.eventType === "SHIPMENT_CONFIRMED")!;
      ...
      const afterE1Seller = await escrowPaymentsService.approveEvent(
        paymentId, escrowItemId, "SHIPMENT_CONFIRMED", "seller",
      );
      const e1s = afterE1Seller.escrows.find((e) => e._id.toString() === escrowItemId)!;
      const approval1s = e1s.approvals.find((a) => a.eventType === "SHIPMENT_CONFIRMED")!;
      const approval2s = e1s.approvals.find((a) => a.eventType === "INSPECTION_PASSED")!;
      ...
      const afterE2Buyer = await escrowPaymentsService.approveEvent(
        paymentId, escrowItemId, "INSPECTION_PASSED", "buyer",
      );
      const e2b = afterE2Buyer.escrows.find((e) => e._id.toString() === escrowItemId)!;
      const approval2b = e2b.approvals.find((a) => a.eventType === "INSPECTION_PASSED")!;
      ...
      const finalPayment = await escrowPaymentsService.approveEvent(
        paymentId, escrowItemId, "INSPECTION_PASSED", "seller",
      );
