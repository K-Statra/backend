...
describe("CompanyImagesService", () => {
  ...
  describe("getImages", () => {
    ...
    it("없는 기업 → NotFoundException", async () => {
      ...
      await expect(service.getImages(COMPANY_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("addImage", () => {
    ...
    it("없는 기업 → NotFoundException", async () => {
      ...
      await expect(
        service.addImage(COMPANY_ID, { url: "http://img.com/a.jpg" }),
  ...
  describe("removeImage", () => {
    it("이미지 삭제 성공", async () => {
      ...
      await expect(
        service.removeImage(COMPANY_ID, IMAGE_ID),
    ...
    it("없는 이미지 ID → NotFoundException", async () => {
      ...
      await expect(service.removeImage(COMPANY_ID, IMAGE_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
