...
import { MatchFeedback, MatchFeedbackDocument } from './schemas/match-feedback.schema';
...
function scoreCompany(buyer: BuyerDocument, company: any): { score: number; reasons: string[] } {
  ...
  if (tagMatches > 0) { score += tagMatches * 2; reasons.push(`tags overlap x${tagMatches}`); }

  if (companyIndustry && buyerIndustries.has(companyIndustry)) { score += 3; reasons.push('industry match'); }

  const needMatches = intersectCount(buyerNeeds, companyOfferings);
  if (needMatches > 0) { score += needMatches * 2; reasons.push(`needs-offerings overlap x${needMatches}`); }

  const days = Math.max(0, (Date.now() - new Date(company.updatedAt || Date.now()).getTime()) / 86400000);
  const recency = Math.max(0, 30 - days) / 30;
  if (recency > 0) { score += recency; reasons.push('recently updated'); }
  ...
  if (autoRegex.test(companyIndustry) || Array.from(companyTags).some(t => autoRegex.test(t))) {
    score += 2.0; reasons.push('Automotive sector priority');
  }

  if (company.dart?.corpCode) { score += 1.5; reasons.push('DART verified'); }
...
export class MatchesService {
  ...
  constructor(
    ...
    @InjectModel(MatchLog.name) private readonly matchLogModel: Model<MatchLogDocument>,
    @InjectModel(MatchFeedback.name) private readonly feedbackModel: Model<MatchFeedbackDocument>,
  ) {}
  ...
  async submitFeedback(companyId: string, dto: { rating: number; comments?: string; locale?: string; source?: string }) {
