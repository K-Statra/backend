...
export class AdminService {
  ...
  async exportPaymentsCsv(query: Pick<ListPaymentsQueryDto, 'status' | 'buyerId' | 'companyId' | 'from' | 'to'>) {
    const filter: Record<string, any> = {};
    ...
    const lines = [
      header.join(','),
      ...items.map((p) =>
        [p._id, p.buyerId, p.companyId, p.amount, p.currency, p.status, p.provider, p.providerRef,
